"use strict";
var __decorate =
  (this && this.__decorate) ||
  function (decorators, target, key, desc) {
    var c = arguments.length,
      r = c < 3 ? target : desc === null ? (desc = Object.getOwnPropertyDescriptor(target, key)) : desc,
      d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function")
      r = Reflect.decorate(decorators, target, key, desc);
    else
      for (var i = decorators.length - 1; i >= 0; i--)
        if ((d = decorators[i])) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
  };
var __metadata =
  (this && this.__metadata) ||
  function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.MarkNoShowOutput = void 0;
const openapi = require("@nestjs/swagger");
const swagger_1 = require("@nestjs/swagger");
const class_transformer_1 = require("class-transformer");
const class_validator_1 = require("class-validator");
const platform_constants_1 = require("@calcom/platform-constants");
class Attendee {
  static _OPENAPI_METADATA_FACTORY() {
    return { email: { required: true, type: () => String }, noShow: { required: true, type: () => Boolean } };
  }
}
__decorate(
  [(0, class_validator_1.IsEmail)(), __metadata("design:type", String)],
  Attendee.prototype,
  "email",
  void 0
);
__decorate(
  [(0, class_validator_1.IsBoolean)(), __metadata("design:type", Boolean)],
  Attendee.prototype,
  "noShow",
  void 0
);
class HandleMarkNoShowData {
  static _OPENAPI_METADATA_FACTORY() {
    return {
      message: { required: true, type: () => String },
      noShowHost: { required: false, type: () => Boolean },
      attendees: { required: false, type: () => [Attendee] },
    };
  }
}
__decorate(
  [(0, class_validator_1.IsString)(), __metadata("design:type", String)],
  HandleMarkNoShowData.prototype,
  "message",
  void 0
);
__decorate(
  [
    (0, class_validator_1.IsBoolean)(),
    (0, class_validator_1.IsOptional)(),
    __metadata("design:type", Boolean),
  ],
  HandleMarkNoShowData.prototype,
  "noShowHost",
  void 0
);
__decorate(
  [
    (0, class_validator_1.ValidateNested)(),
    (0, class_transformer_1.Type)(() => Attendee),
    (0, class_validator_1.IsArray)(),
    (0, class_validator_1.IsOptional)(),
    __metadata("design:type", Array),
  ],
  HandleMarkNoShowData.prototype,
  "attendees",
  void 0
);
class MarkNoShowOutput {
  static _OPENAPI_METADATA_FACTORY() {
    return {
      status: { required: true, type: () => Object },
      data: { required: true, type: () => HandleMarkNoShowData },
    };
  }
}
__decorate(
  [
    (0, swagger_1.ApiProperty)({
      example: platform_constants_1.SUCCESS_STATUS,
      enum: [platform_constants_1.SUCCESS_STATUS, platform_constants_1.ERROR_STATUS],
    }),
    (0, class_validator_1.IsEnum)([platform_constants_1.SUCCESS_STATUS, platform_constants_1.ERROR_STATUS]),
    __metadata("design:type", Object),
  ],
  MarkNoShowOutput.prototype,
  "status",
  void 0
);
__decorate(
  [
    (0, swagger_1.ApiProperty)({
      type: HandleMarkNoShowData,
    }),
    (0, class_validator_1.ValidateNested)(),
    (0, class_transformer_1.Type)(() => HandleMarkNoShowData),
    __metadata("design:type", HandleMarkNoShowData),
  ],
  MarkNoShowOutput.prototype,
  "data",
  void 0
);
exports.MarkNoShowOutput = MarkNoShowOutput;
//# sourceMappingURL=mark-no-show.output.js.map
